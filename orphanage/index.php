...
{
	$query=mysql_query("select * from o_details where email='" .$_POST['user']. "' and orpPass='" .$_POST['pass'] ."'");
	...
	if($rows==0)
		...
		if($fetch['orpPass']==$_POST['pass'])
			{	
				$_SESSION['orphanage'] = $fetch['name'];
				$_SESSION['orphanageid'] = $fetch['id'];
		$sql=mysql_query("insert into logbook (date,userid,category,task) values('".$today."','".$_SESSION['adminid']."','login','".$_SESSION['admin']." login ')");
...
<body>
<div class="logdic">
<div style="width:100%;text-align:center; padding:15px"><h4>Orphanage Login</h4></div>
<form method="post">
