...
$sql=mysql_query(" insert into applicants(name,qualification,imgPath_home) values('".$_POST['title']."' , '".$_POST['description']."' , '".basename( $_FILES['file']['name'])."')");

		header('Location: orphanages.php?mode=show&msg=added');
}
	$req=isset($_REQUEST['action']) ? $_REQUEST['action'] :'';
if($req=='edit')
{
$sqlget=mysql_query("select * from applicants where id=".$_GET['id']);
...
mysql_query("update applicants set name='".$_POST['title']."',qualification='".$_POST['description']."',imgPath_home='".$image1."' where id=".$_GET['id']);
header('Location:orphanages.php?mode=show&msg=edited');
	}
	$req=isset($_REQUEST['action']) ? $_REQUEST['action'] :'';
if($req=='delete')
{
		mysql_query("delete from applicants where id=".$_GET['id']);
		header('Location:orphanages.php?mode=show&msg=deleted');
	}
?>
	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li><a href="orphanages.php">Orphanages </a></li>
			</ul>
...
?>
	...
	<tbody>
	<?PHP	
					  $sqlabout=mysql_query("select * from applicants order by id " ); 
					...
					while($resultabout=mysql_fetch_array($sqlabout))
					{
					 ?>
                      <tr bgcolor="#E1E1E1">
						...
						<a class="btn btn-info" href="orphanages.php?mode=edit&amp;id=<?php echo $resultabout['id']; ?>">Edit
						<i class="halflings-icon white edit"></i>  
									</a>					
						<a class="btn btn-danger" href="orphanages.php?action=delete&amp;id=<?php echo $resultabout['id']; ?>" onClick="return confirm('Are you sure you want to delete this data?');">Delete
	...
	<tfoot>
    	<tr>
        	...
        	<td class="rounded-foot-right"><a href="orphanages.php?mode=add" class="bt_green"><span class="bt_green_lft"></span><strong>Add new records</strong><span class="bt_green_r"></span></a></td>
        </tr>
    </tfoot>
       </table>
	   </div>  
	...
	<div class="left_content">
				...
				?>	
				<div class="box-content">
					<form action="orphanages.php?action=add" method="post" class="form-horizontal" enctype="multipart/form-data" name="form1" id="form1" onSubmit="return check();">
					<fieldset>
				  <div class="control-group">
					...
					<div class="controls"><input name="description" type="text" class="button" size="60"
	...
	<div class="left_content">
				...
				?>	
				<div class="box-content">
					<form action="orphanages.php?action=edit&id=<?PHP echo $_GET['id']; ?>" method="post" class="form-horizontal" enctype="multipart/form-data" name="form1" id="form1" onSubmit="return check();">
					<fieldset>
				  <?PHP
				  $about=mysql_query("select * from applicants where id=" . $_GET['id']);
				  ...
				  <div class="control-group">
					...
					<div class="control-group">
					<label class="control-label" for="typeahead">Details: </label>
					<div class="controls"><textarea name="description" type="text" class="button" rows="5" cols="30" ><?PHP echo $about_data['qualification']; ?></textarea>
					</div>
					</div>
